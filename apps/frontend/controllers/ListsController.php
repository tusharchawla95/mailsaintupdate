...
class ListsController extends Controller
{
    ...
    public function actionSubscribe_confirm($list_uid, $subscriber_uid, $do = null)
    {
        ...
        if ($do === 'subscribe-back' && $list->opt_in === Lists::OPT_IN_DOUBLE) {
        	...
        	$subscriber->createOptinHistory();
	        $this->callbacks->_sendSubscribeConfirmationEmail(new CEvent($this->callbacks, array(
		        'instances'     => array(),
		        'subscriber'    => $subscriber,
		        'list'          => $list,
		        'action'        => 'subscribe',
    ...
    public function actionUnsubscribe($list_uid, $subscriber_uid = null, $campaign_uid = null, $type = null)
    {
	    ...
	    if ($request->isPostRequest && empty($_POST[$subscriber->modelName]['email']) && !empty($_subscriber) && $request->getQuery('source') === 'email-client-unsubscribe-button') {
		    $_POST[$subscriber->modelName]['email'] = $_subscriber->email;
    ...
    public function actionBlock_address()
    {
        ...
        if ($request->isPostRequest && ($attributes = (array)$request->getPost($model->modelName, array()))) {
	        ...
	        // since 1.9.30
	        Yii::app()->hooks->doAction('controller_action_before_save_data', $collection = new CAttributeCollection(array(
		        'controller'=> $this,
