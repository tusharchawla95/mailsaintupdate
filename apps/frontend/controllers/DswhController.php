...
class DswhController extends Controller
{
	...
	public function actionPmta()
	{
		...
		$parsedEvents = (array)CJSON::decode($postedEvents);
		...
		if (!empty($parsedEvents) && array_key_exists('type', $parsedEvents)) {
			$parsedEvents = array($parsedEvents);
		}
    ...
    protected function processMailgunV2()
    {
        ...
        $metaData  = CJSON::decode($eventData['user-variables']['metadata']);
        ...
        if ($event == 'failed') {
            ...
            $bounceMessage = CJSON::encode(array(
                'message'     => $message,
            ...
            $bounceLog->campaign_id   = $campaign->campaign_id;
            $bounceLog->subscriber_id = $subscriber->subscriber_id;
            $bounceLog->message       = $bounceMessage;
            $bounceLog->bounce_type   = $bounceType;
            $bounceLog->save();
        ...
        if (in_array($event, array('complained', 'unsubscribed'))) {

            if ($event === 'complained' && Yii::app()->options->get('system.cron.process_feedback_loop_servers.subscriber_action', 'unsubscribe') == 'delete') {
                $subscriber->delete();
                Yii::app()->end();
            }
    ...
    public function processSparkpost()
    {
        ...
        foreach ($events as $evt) {
            ...
            if (in_array($evt['type'], array('bounce', 'out_of_band'))) {
