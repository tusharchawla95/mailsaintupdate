...
class DashboardController extends Controller
{
    ...
    public function actionIndex()
    {
	    ...
	    if ($options->get('system.updater.servers.sendinblueapiupgradev2tov3.info', 0) == 0) {
		    ...
		    $serversCount = DeliveryServerSendinblueWebApi::model()->countByAttributes(array(
		    ...
		    if (!empty($serversCount)) {
			    DeliveryServerSendinblueWebApi::model()->updateAll(array(
			    	'status' => DeliveryServerSendinblueWebApi::STATUS_INACTIVE,
			    ), '`type` = :type AND `status` = :status', array(
				    ':type'   => DeliveryServerSendinblueWebApi::TRANSPORT_SENDINBLUE_WEB_API,
				    ':status' => DeliveryServerSendinblueWebApi::STATUS_ACTIVE,
			    ));
			    $notify->addInfo(Yii::t('servers', 'Because of the API upgrade, all SendInBlue delivery servers have been disabled. Please update their API keys and validate them again.'));
		    }
	    }
	    //
	    ...
	    if (Yii::app()->params['console.save_command_history']) {
		    ...
		    $commandMap = Yii::app()->hooks->applyFilters('backend_dashboard_console_command_history_check_list', $commandMap, $this);
		    foreach ($commandMap as $commandName => $seconds) {
			    if (!ConsoleCommandList::isCommandActive($commandName, $seconds)) {
				    $notify->addWarning(Yii::t('app', 'The "{command}" command did not run in the last {num}. Please check your cron jobs and make sure they are properly set!', array(
					    '{command}' => $commandName,
