...
class ListSubscriber extends ActiveRecord
{
	...
	public function updateGeoLocationFields()
	{
		...
		if (!isset($fields[(int)$this->list_id])) {
			$fields[(int)$this->list_id] = array();
			$criteria = new CDbCriteria();
			$criteria->select = 't.field_id, t.tag';
			$criteria->with['type'] = array(
				'joinType' => 'INNER JOIN',
				'together' => true,
			);
			$criteria->compare('t.list_id', (int)$this->list_id);
			$criteria->addInCondition('type.identifier', array('geocity', 'geocountry', 'geostate'));
		...
		foreach ($fields[(int)$this->list_id] as $field) {
			...
			if (empty($value)) {
				...
				if (empty($value['value'])) {
					$command->update('{{list_field_value}}', $data, 'value_id = :vid', array(':vid' => $value['value_id']));
				}
			}
		}
	}
