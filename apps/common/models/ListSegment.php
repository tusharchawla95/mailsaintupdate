...
class ListSegment extends ActiveRecord
{
    ...
    protected function _createCountFindSubscribersCriteria(array $params = array())
    {
        ...
        // since 1.9.12
	    $campaignConditions = ListSegmentCampaignCondition::model()->findAllByAttributes(array(
	    ...
	    if (!empty($campaignConditions)) {
		    foreach ($campaignConditions as $condition) {
				if ($condition->getIsOpenAction() && !$md->hasRelation($condition->getTimeComparisonAliasForDb())) {
					$md->addRelation($condition->getTimeComparisonAliasForDb(), array(ListSubscriber::HAS_MANY, 'CampaignTrackOpen', 'subscriber_id'));
					continue;
				}
			    if ($condition->getIsClickAction() && !$md->hasRelation($condition->getTimeComparisonAliasForDb())) {
				    $md->addRelation($condition->getTimeComparisonAliasForDb(), array(ListSubscriber::HAS_MANY, 'CampaignTrackUrl', 'subscriber_id'));
				    continue;
			    }
		    }	
	    }
        ...
        foreach ($campaignConditions as $condition) {
	        ...
	        if ($condition->getIsOpenAction() || $condition->getIsClickAction()) {
		        if (!empty($condition->campaign_id)) {
			        $campaignIds = array((int)$condition->campaign_id);
		        } else {
	        ...
	        if ($condition->getIsOpenAction()) {
		        $with[$condition->getTimeComparisonAliasForDb()] = array(
			        'select'   => false,
			        'together' => true,
			        'joinType' => 'INNER JOIN',
		        );
