...
class DeliveryServer extends ActiveRecord
{
    ...
    public function rules()
    {
        $rules = array(
            ...
            array('from_email', '_validateFromEmail'),
    ...
    */
	 ...
	 * @param $params
	 */
    public function _validateFromEmail($attribute, $params)
    {
    	...
    	$criteria->addNotInCondition('server_id', array((int)$this->server_id));
	    $criteria->addNotInCondition('customer_id', array((int)$this->customer_id));
    	$count = (int)self::model()->count($criteria);
    	if ($count === 0) {
    		return;
	    }

	    $this->addError($attribute, Yii::t('servers', 'This email address cannot be used.'));
    }
