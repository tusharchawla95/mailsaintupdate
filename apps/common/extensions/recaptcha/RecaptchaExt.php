...
class RecaptchaExt extends ExtensionInit 
{
    ...
    public function run()
    {
		...
		Yii::app()->params['extensions.recaptcha.data.enabled_for_block_email_form']    = $this->getOption('enabled_for_block_email_form') == 'yes';
	    ...
	    if ($this->isAppName('frontend') && Yii::app()->params['extensions.recaptcha.data.enabled_for_block_email_form']) {
		    $appName = Yii::app()->apps->getCurrentAppName();
		    Yii::app()->hooks->addAction($appName . '_controller_lists_before_action', array($this, '_listsBlockAddressAction'));
	    }
	...
	public function _listsBlockAddressAction($action)
	{
		if (!in_array($action->id, array('block_address'))) {
			return;
		}

		$canShow = Yii::app()->params['extensions.recaptcha.data.enabled_for_block_email_form'];
		if (!$canShow) {
			return;
		}
		...
		Yii::app()->hooks->addAction('controller_action_before_save_data', array($this, '_listsBlockAddressProcessForm'));
		Yii::app()->hooks->addAction('after_active_form_fields', array($this, '_listsBlockAddressFormAppendHtml'));
	}

	/**
	 * @param $collection
	 */
	public function _listsBlockAddressProcessForm($collection)
	{
	 ...
	 * @param $collection
	 */
	public function _listsBlockAddressFormAppendHtml($collection)
	{
		$append  = sprintf('<div class="row"><hr /><div class="col-lg-12"><div class="pull-right g-recaptcha" data-sitekey="%s"></div></div></div>', Yii::app()->params['extensions.recaptcha.data.site_key']);
		$append .= '<div class="clearfix"><!-- --></div>';
		echo $append;
	}
