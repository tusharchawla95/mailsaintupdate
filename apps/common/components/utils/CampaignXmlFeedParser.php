...
class CampaignXmlFeedParser
{
    ...
    public static function parseContent($content, Campaign $campaign, ListSubscriber $subscriber = null, $cache = false, $cacheKeySuffix = null, DeliveryServer $server = null)
    {
        ...
        foreach ($multiMatches[0] as $fullFeedHtml) {
            ...
            $doCache   = !$campaign->isDraft && $cache;
	 ...
	 * @throws Exception
	 */
    public static function getRemoteFeedItems($url, $count = 10, Campaign $campaign = null, $cache = false, $offset = 0, $daysBack = -1, $noItemAction = '', $sendOnlyUniqueItems = false)
    {
    	if (empty($campaign)) {
    		return array();
	    }
