        ...
        try {
	        ...
	        if ($currentPercent < $minimumPercent) {
	        	throw new Exception(
			        ...
			        $queueTableSubscribersCount, $originalTableSubscribersCount, $currentPercent, $minimumPercent 
			        )
		        );
	        } 
	        */
    ...
    protected function _applyOpenUnopenPreviousCampaignsCriteria(array $subscribers = array())
    {
    	...
    	foreach ($subscribers as $index => $subscriber) {
    		...
    		foreach ($openUnopenFilters as $filter) {
    			$hasOpened = $subscriber->hasOpenedCampaignById($filter->previous_campaign_id);
