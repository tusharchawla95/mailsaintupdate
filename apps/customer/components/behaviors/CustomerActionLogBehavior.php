...
class CustomerActionLogBehavior extends CBehavior
{
     ...
     * @return bool
     */
    public function surveyExportStart($survey, $export)
    {
        if (empty($this->owner->customer_id)) {
            return false;
        }
        ...
        $url = Yii::app()->options->get('system.urls.customer_absolute_url');
        $url = $url . sprintf('surveys/%s/overview', $survey->survey_uid);

        $message  = 'The export process for survey "{surveyName}" has successfully started, counting {rowsCount} records!';
        $surveyLink = CHtml::link($survey->name, $url);
        $message  = Yii::t('survey_export', $message, array('{surveyName}' => $surveyLink, '{rowsCount}' => $export->count));
     ...
     * @return bool
     */
    public function surveyExportEnd(Survey $survey, SurveyCsvExport $export)
    {
        if (empty($this->owner->customer_id)) {
            return false;
        }
